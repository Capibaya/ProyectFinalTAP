...
public class ReservacionDAO implements GenericDAO<Reservacion> {
    ...
    public void guardar(Reservacion r) {
        String sql = "INSERT INTO reservaciones (id_cliente,
        id_habitacion, fecha_reservacion, fecha_entrada, fecha_salida,
                numero_huespedes, total, id_estado_reservacion) VALUES
                (?,?,?,?,?,?,?,?)";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, r.getCliente().getIdCliente());
            ps.setInt(2, r.getHabitacion().getIdHabitacion());
            ps.setTimestamp(3,
                    Timestamp.valueOf(r.getFechaReservacion()));
            ...
            ps.setInt(8,
                    r.getEstadoReservacion().getIdEstadoReservacion());
            ps.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Error al guardar reservacion: " +
                    e.getMessage());
        }
    }
    @Override
    public Reservacion buscarPorId(int id) {
        String sql = "SELECT r.*, c.nombre AS cli_nombre, c.apellido AS
        cli_apellido, " +
        "h.numero AS hab_numero, e.nombre AS estado_nombre "
                +
                "FROM reservaciones r " +
                "JOIN clientes c ON r.id_cliente = c.id_cliente " +
                "JOIN habitaciones h ON r.id_habitacion =
        h.id_habitacion " +
        "JOIN estados_reservacion e ON
        r.id_estado_reservacion = e.id_estado_reservacion " +
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.err.println("Error al buscar reservacion: " +
                    e.getMessage());
        }
        return null;
    }
    ...
    public List<Reservacion> obtenerTodos() {
        ...
        try (Statement st = con.createStatement();
            ...
            System.err.println("Error al obtener reservaciones: " +
                    e.getMessage());
        }
        return lista;
    }
    public List<Reservacion> obtenerPorFechas(LocalDate desde, LocalDate
            hasta) {
        List<Reservacion> lista = new ArrayList<>();
        String sql = "SELECT r.*, c.nombre AS cli_nombre, c.apellido AS
        cli_apellido, " +
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.err.println("Error al filtrar reservaciones: " +
                    e.getMessage());
        }
        return lista;
    }
    ...
    public void actualizar(Reservacion r) {
        String sql = "UPDATE reservaciones SET id_estado_reservacion=?,
        total=? WHERE id_reservacion=?";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1,
                    r.getEstadoReservacion().getIdEstadoReservacion());
            ...
            System.err.println("Error al actualizar reservacion: " +
                    e.getMessage());
        }
    }
    @Override
    public void eliminar(int id) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.err.println("Error al eliminar reservacion: " +
                    e.getMessage());
        }
    }
    private Reservacion mapear(ResultSet rs) throws SQLException {
        ...
        r.setFechaReservacion(rs.getTimestamp("fecha_reservacion").toLocalDateTim
                e());
